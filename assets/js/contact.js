class CommissionForm {
      ...
      Object.keys(rules).forEach((fieldId) => {
        ...
        if (field) {
          field.addEventListener("blur", () =>
            this.validateField(fieldId, tabName)
          );
  ...
  validateAndPreviewFile(file) {
    ...
    if (!allowedTypes.includes(file.type)) {
      this.showError(
        "file-error",
        "Please upload only JPEG, JPG, or PNG images"
      );
      this.clearFileInput();
      return;
    }
    ...
    reader.onload = (e) => {
      filePreview.innerHTML = `
                ...
                <div class="file-info">
                    <span>${file.name} (${this.formatFileSize(
        file.size
      )})</span>
  ...
  updateCharacterCount(fieldId) {
    ...
    if (!field || !countElement) {
      console.log(
        `Character count elements not found: field=${fieldId}, countElement=${countElementId}`
      );
      return;
    }
  ...
  validateField(fieldId, tabName) {
    ...
    if (fieldId === "terms-agreement") {
      ...
      if (isValid && rule.minLength && field.value.length < rule.minLength) {
        isValid = false;
        errorMessage = `${this.getFieldLabel(fieldId)} must be at least ${
          rule.minLength
        } characters`;
      }
      ...
      if (isValid && rule.maxLength && field.value.length > rule.maxLength) {
        isValid = false;
        errorMessage = `${this.getFieldLabel(fieldId)} must be no more than ${
          rule.maxLength
        } characters`;
      }
  ...
  updateValidationSummary(errors) {
    if (errors.length > 0) {
      this.errorList.innerHTML = errors
        .map((error) => `<li>${error} field has errors</li>`)
        .join("");
  ...
  updateProgress() {
    ...
    console.log(
      `Progress Update - Tab: ${this.currentTab}, Completed: ${completedFields}/${fields.length}, Weight: ${completedWeight}/${totalWeight}, Progress: ${progress}%`
    );
  }
  ...
  async handleSubmit(e, tabName) {
    ...
    if (tabName === "commission" && this.availableSlots <= 0) {
      alert(
        "Sorry, commissions are currently closed. Please check back later."
      );
      return;
    }
    ...
    try {
        ...
        if (tabName === "commission") {
          ...
          if (fileInput && fileInput.files.length > 0) {
            ...
            formData.append(
              ...
              `File name: ${file.name}, Size: ${this.formatFileSize(
                file.size
              )}, Type: ${file.type}`
            );
            formData.append(
              "Note",
              "Reference image was selected but not uploaded due to API limitations. Please send the image via email if needed."
            );
          }
        }
      ...
      formData.append(
        ...
        this.fileUploadEnabled
          ? "2.0 - Enhanced (With File Upload)"
          : "2.0 - Enhanced (No File Upload)"
      );
      ...
      this.handleError(
        `There was an error submitting your request: ${error.message}. Please try again or contact us directly.`
      );
    } finally {
  ...
  handleSuccess(tabName) {
    ...
    let successMessage = {
      commission:
        "Thank you for your commission request! You'll receive a confirmation email shortly.",
      inquiry:
        "Thank you for your inquiry! We'll get back to you within 24-48 hours.",
      support:
        "Thank you for contacting support! We'll help resolve your issue as soon as possible.",
    };
    ...
    if (tabName === "commission") {
      ...
      if (fileInput && fileInput.files.length > 0) {
        if (this.fileUploadEnabled) {
          successMessage[tabName] +=
            " <br><br><strong>✅ Your reference image has been uploaded successfully!</strong>";
        } else {
          successMessage[tabName] +=
            " <br><br><strong>🔧 Note:</strong> Your reference image information was recorded. Please send the actual image file to kyuushi992@gmail.com for your commission.";
        }
      }
    }
...
function showQR(type) {
  ...
  const selectedTab = document.querySelector(
    `.qr-tab:nth-child(${getTabIndex(type)})`
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
      const currentForm =
        window.commissionForm.forms[window.commissionForm.currentTab];
      const submitBtn =
        window.commissionForm.submitBtns[window.commissionForm.currentTab];
