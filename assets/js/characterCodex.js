...
function updateBackground() {
  ...
  setTimeout(() => {
    const bg =
      characterBackgrounds[currentCharacter]?.[backgroundType] ??
...
function getCharacterDetails(character) {
  ...
  if (accent) {
    ...
    if (characterOverlay)
      characterOverlay.style.setProperty("--accent", accent);
  }
  ...
  const defaultDetails = {
    ...
    ability2:
      "<strong><i class='fas fa-star'></i> Ability 2</strong> <p>Description coming soon.</p>",
    ability3:
      "<strong><i class='fas fa-star'></i> Ability 3</strong> <p>Description coming soon.</p>",
    ability4:
      "<strong><i class='fas fa-star'></i> Ability 4</strong> <p>Description coming soon.</p>",
    ability5:
      "<strong><i class='fas fa-star'></i> Ability 5</strong> <p>Description coming soon.</p>",
    ability6:
      "<strong><i class='fas fa-star'></i> Ability 6</strong> <p>Description coming soon.</p>",
    background:
      "<p><i class='fas fa-book'></i> Background details coming soon.</p>",
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (characterContainer) {
    characterContainer.style.backgroundImage =
      characterBackgrounds.comingSoon.landscape ||
      "url('assets/images/bg/default.webp')";
  }
...
function applyCombinedFilters() {
  ...
  const activeRole =
    activeBtn && (activeBtn.dataset.category || "all")
      ? (activeBtn.dataset.category || "all").toString().trim().toLowerCase()
      : "all";
  ...
  const checkedFactions = checkboxes
    ...
    .map((cb) => (cb.id || cb.value || "").toString().trim().toLowerCase())
    .filter(Boolean);
  const anyFactionChecked = checkedFactions.length > 0;

  document.querySelectorAll(".character-card").forEach((card) => {
    const cardRoles = (card.dataset.category || "")
      .split(",")
      .map((s) => s.trim().toLowerCase())
      .filter(Boolean);
    const cardFactions = (card.dataset.faction || "")
      .split(",")
      .map((s) => s.trim().toLowerCase())
      .filter(Boolean);
    ...
    const factionMatch =
      !anyFactionChecked ||
      cardFactions.some((g) => checkedFactions.includes(g));
...
document
  .querySelector(".character-cards-wrapper")
...
function renderCharacterDetails(details = {}) {
  ...
  const elementalImgs =
    (d.elementalImages || [])
      ...
      .join("") ||
    `<img src="assets/images/character-presets/roles/question.webp" alt="" title="Unknown">`;
